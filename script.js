...
============================================================ */
const LT_START_DATE = new Date("2025-12-08T00:20:00"); // edit to your real date
...
let auroraResetFn = null;

function initAuroraScreen(){
  if(auroraInited){
    if(auroraResetFn) auroraResetFn();
    return;
  }
  auroraInited = true;
  ...
  function resetAuroraWalk(){
    auroraProgress = 0;
    auroraWalkingActive = true;

    camera.position.copy(camStart.pos);
    currentLook.copy(camStart.look);
    camera.lookAt(currentLook);

    const hint = document.getElementById('aurora-tap-hint');
    hint.textContent = 'Tap the screen to walk forward...';
    hint.style.opacity = '1';

    // clear out anything left over from a previous visit so it doesn't get stuck
    if(starSprite){ scene.remove(starSprite); starSprite = null; }
    starArrived = false;
    starGlowT = 0;
